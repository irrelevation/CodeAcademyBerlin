...
// Exercise 1
let names = ["Annika", "Stephan", "Anan", "Andreas", "Lukas", "Ottavia", "Lucas",];
names.sort();
console.log(names[0]);
...
printRepeatingElements([3, 6, 67, 6, 23, 11, 100, 8, 93, 0, 17, 24, 7, 1, 33, 45, 28, 33, 23, 12, 99, 100]);
...
Number.prototype.reverse = function() {
    if (this < 0) {
        return Number(`-${String(this * -1).split("").reverse().join("")}`);
    }
...
function getLongestWord(phrase) {
    return  phrase.split(/\b/).reduce((previous, current) => previous.length > current.length ? previous : current);
}

console.log(getLongestWord("Hello, this is a serntence to test if this function returns 'Schifffahrtsgesellschaft."));
